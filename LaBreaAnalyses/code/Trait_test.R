...
colnames(taxon) <- c('RevisedName', "Order", "Family", "Genus", "Precip_affinity", "Temp_affinity", "Diet_group", "Diet_breadth") 

for (i in 1:nrow(taxon)){
  taxon[i,] <- taxonomy_traits[which(taxonomy_traits$'OriginalName' == mammals$prelim_taxon_name[i]),
  c('RevisedName', "Order", "Family", "Genus", "Precip_affinity", "Temp_affinity", "Diet_group", "Diet_breadth")] 
} #error here
